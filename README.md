...
### **Configure OCR keys first**

The OCR workflow relies on a cloud model. Before running scans locally, create a `.env` file (or use your preferred secrets manager) and provide at least one of the keys below:

- `EXPO_PUBLIC_OPENAI_API_KEY` – recommended. Uses OpenAI's GPT-4o-mini Vision API.
- `EXPO_PUBLIC_OCR_SPACE_API_KEY` – optional fallback if you prefer OCR.Space.

Restart the Expo dev server after adding environment variables so the app can read them.
