...
import CameraViewWrapper from "./components/CameraViewWrapper";
...
export default function App() {
  ...
  // Track DB initialization error (non-blocking)
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Store a short message for the UI so child components can present
        // a clearer error to the user if needed.
        try {
          setDbError(typeof err === 'string' ? err : (err && (err as any).message) ? (err as any).message : String(err));
        } catch (e) {
  ...
  files: selectedFilesData,
    ...
    return (
      <SafeAreaView className="flex-1 bg-background">
        ...
        {showCamera ? (
          <CameraViewWrapper onClose={() => setShowCamera(false)} onPhotoTaken={handlePhotoTaken} />
        ) : null}
